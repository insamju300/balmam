...
$(document).on("change", ".hidden-upload", function() {
	...
	$.ajax({
		...
		success: function(response) {
			...
			$("#form_profile_image_id").val(mediaFileDto.id);
			//todo 에러처리
...
function emailValidationCheck() {
	...
	if (email === "") {
		...
		validationMessage = "유효하지 않은 emial 형식입니다.";
		      ...
		      isEmailDuplicated(email).then(function(response) {
...
function nicknameValidationCheck() {
	...
	if (nickname === "") {
      ...
      isNicknameDuplicated(nickname).then(function(response) {
            if (response) {
                validationMessage = "이미 등록되어있는 닉네임입니다.";
...
async function isEmailDuplicated(email) {
	let response = await $.get("/member/isExistsEmail", { email: email });
	 return response;
}

async function isNicknameDuplicated(nickname) {
    let response = await $.get("/member/isExistsNickname", { nickname: nickname });
    return response;
}
